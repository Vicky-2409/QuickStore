...
const MONGO_URI = process.env.MONGO_URI || "mongodb+srv://svignesh2409:GOgxz3UEBjCrSNtl@cluster0.hchjyx3.mongodb.net/quick-store-payment-service";
// Try different RabbitMQ URLs if connection fails
const RABBITMQ_URL = process.env.RABBITMQ_URL || "amqp://rabbitmq-service:5672";
const RAZORPAY_KEY_ID = process.env.RAZORPAY_KEY_ID || "rzp_test_Wsp2NzIUWHF2Cm";
...
async function setupServices() {
  ...
  while (retries < maxRetries) {
    try {
      console.log(`Attempt ${retries + 1} to connect to RabbitMQ at ${RABBITMQ_URL}...`);
      ...
      const paymentService = new PaymentService(paymentRepository, channel);
      ...
      console.error(`Service initialization attempt ${retries + 1} failed:`, error);
      retries++;
